...
| Error on any automatic arm/disarm | an automatic attempt failed, carrying the panel's error text (`FAILED`). Presence auto-arm being unable to fire at all because a tracked person's presence is stuck `home` (issue #533 — names who, and since when) is a **separate, always-off-Telegram** condition (issue #626 — expected, frequent noise): it never pages regardless of this toggle. It is still shown in the Presence card immediately and always logged to the local activity log — first once the block has persisted for `arm_block_notify_after_s` (default 900 s, `config/presence_automation.json`, below which it's just two people walking in a few seconds apart, issue #599), then again roughly every 5 minutes for as long as it remains stuck. |
...
| `PC_FLEET_HUB_BASE` | Optional, default `http://127.0.0.1:8000`; base URL of the local-llm-hub whose machine API the PC-fleet shutdown/wake proxies use (#498). |
...
On a brand-new session (no cached session token at all) Apple may require 2FA during the fetch. If the command reports that, approve the sign-in on a trusted Apple device and rerun with the displayed code — targeting the account, since 2FA is per Apple ID:
...
```powershell
& .\.venv\Scripts\python.exe -m src.list_presence --account 2 --renew-trust
```
...
**Re-authenticating after a login failure (issue #442).** `GET /api/presence` diagnostics (`reason`) tell you what broke: `2fa_required` (Apple refused the Find My fetch for that account — rerun the 2FA smoke command above; a merely *untrusted* session that still serves devices is reported healthy, see #658 below), `not_configured` (`.env` is missing `ICLOUD_EMAIL`/`ICLOUD_PASSWORD`), `error` with a detail message, or `partial` (one of two accounts is broken while the other is healthy — see the `accounts[]` breakdown for which). A detail of *"Invalid email/password combination"* usually means the Apple ID password changed since the session was trusted — update `ICLOUD_PASSWORD` (or `ICLOUD_PASSWORD_2`) in `.env` and rerun `src.list_presence --account <N>`. If Apple's response instead says the account was **locked for security reasons** (a real failure mode: the background refresher retries every `PRESENCE_ICLOUD_REFRESH_INTERVAL_S`, so a stale password can trip Apple's own repeated-failed-login lock), that's an Apple-side hold with no code-side fix — unlock/reset the account at [iforgot.apple.com](https://iforgot.apple.com) first, then retry the CLI. `POST /api/presence/refresh` (or the CLI) re-runs the same login once the credentials/account are good again.

**Self-heal + Telegram heads-up on re-auth (issue #655, #657).** A broken account (`2fa_required`/`error`) no longer needs a manual `tray.bat --restart` to recover — the refresher retries a fresh Apple sign-in on its own, at most once per `PRESENCE_ICLOUD_RETRY_BACKOFF_S` (default 4h) *from the last forced retry, not from the last recovery* — a flapping account stays throttled instead of every recovery resetting the clock (#656) — deliberately much slower than the normal `PRESENCE_ICLOUD_REFRESH_INTERVAL_S` poll so this can never regress into #651's every-poll re-authentication (which was itself triggering repeated "someone is trying to access your account" prompts). Right before each retry attempt, and again once the account recovers, a Telegram message goes out naming the account (`ICLOUD_LABEL`/`ICLOUD_LABEL_2`, e.g. "Roberto"/"Ana" — falls back to the Apple ID email when unset, not a bare "account 1"/"account 2", so the message is always identifiable) via the same notifier as the alarm/UPS alerts (`config/notify_config.json` or `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`). The point is context: an unexplained Apple sign-in push is alarming, but "Reconnecting Roberto's iCloud Find My — approve the sign-in if prompted" turns it into an expected, safe-to-approve event. A healthy account is untouched by any of this — same caching/cadence as before #655, and no notification ever fires for a routine poll.
...
| `PRESENCE_ICLOUD_RETRY_BACKOFF_S` | Optional, default `14400` (4h); minimum seconds between self-heal retry attempts (and their Telegram announcement) for an account stuck in `2fa_required`/`error` (#655). Healthy accounts are unaffected. |
| `ICLOUD_LABEL` / `ICLOUD_LABEL_2` | Optional friendly names (e.g. `Roberto` / `Ana`) used only in the self-heal Telegram messages above; unset falls back to that account's Apple ID email. |
...
**Staleness corroboration (issue #653).** The Shortcut webhook path is edge-triggered only (Arrive/Leave geofence crossings, no periodic heartbeat), so a person who simply stays continuously home or away past `stale_after_s` goes stale even though nothing is broken — and staleness in *any* tracked person used to silently stop the automation from acting on *anyone*, with zero alert. To fix that, a stale person's last known state can now be corroborated by their iCloud/Find My diagnostic entity instead of unconditionally blocking: if that entity's `last_seen` is within `icloud_corroboration_window_s` (default `21600`, 6h; `config/presence_automation.json`) **and** its home/away reading agrees with the stale webhook state, the person still counts as fresh. The link between a webhook person id and its iCloud entity is automatic — a case-insensitive match against the entity's display name (e.g. person id `roberto` matches the iCloud entity displayed as "Roberto"), reusing whatever names are already configured. No corroborating entity, a corroboration read that's itself stale, or a source disagreement all fail safe exactly as before (still blocks) — but a block that persists past `arm_block_notify_after_s` now pages the same Telegram alert as the existing "someone's still home" diagnostic, instead of staying silent forever.
...
Issue #125 added a read-only spike for the **Network** view; issue #129 builds the tab itself, in phases. **Phase 1 (live now):** the **📶 Network** tab sits between Plugs and Security and shows internet/WiFi/LAN health, the attached-device inventory grouped by band (weakest signal first), AP/router health, and the confirm-gated **Reboot AP** — so the network can be watched and managed without logging into the vendor web UIs by hand. The router-reboot button is present but disabled until Phase 3 wires the router data-read. **Phase 2 (live now):** device identity + rename — each device row shows a category glyph and a friendly label (custom name → OUI vendor → reported hostname → MAC), and tapping it opens a detail modal (vendor, IP, band, signal, SSID, MAC) with a rename field. Vendor comes from a bundled, trimmed OUI table (offline, slightly stale — an unknown prefix just falls back to the hostname/MAC); modern phones rotate a randomised MAC per network, so those are flagged and left un-vendored. **Phase 3 (live now):** the router card fills in with the live **WAN/internet status** (WAN up/down, public IP, default gateway, DNS, connection name, link uptime) read from the ZTE web API, and the **Reboot** button is enabled (confirm-gated, ~5-min outage). **Phase 4 (live now):** device **history & smart alerts** — a per-MAC registry (`network_history.py`) remembers each device, so the list shows **online/offline** state with "last seen Xh ago" on absent devices, an offline toggle (persisted, only shown when it has something to reveal, and always labelled with what's currently visible — **Offline shown** / **Offline hidden**, never the pending action), a **new** badge for devices first seen in the last 24 h, and a **Mark important** switch in the detail modal. Both events that history makes visible — a never-before-seen device joining, an *important* device dropping offline — are surfaced on the device's **own row** (the **new** badge, the dimmed offline row); the separate alert strip that restated them was removed with the tab's mobile-layout tightening, and `GET /api/network` no longer returns an `alerts` key (#632). **Network cleanup (live now):** attached devices also have a persisted **Hidden** switch in the detail modal; hidden devices stay out of the default list, show as dimmed when the header **Show hidden** pill is active, and remain separate from delete/history pruning. **Wi-Fi diagnostics (live now):** a collapsible tile reads the dashboard PC's own Windows Wi-Fi interface (`netsh wlan`) and shows the current SSID/signal/channel plus visible BSSIDs, signal %, band/channel, emitter MAC, and separate 2.4/5 GHz channel-overlap charts in the same translucent line/area style as the solar charts. Tapping a Wi-Fi row opens a detail modal with custom label, original SSID/BSSID, security/channel/signal, and a persisted **Hidden** switch; labels and chart tooltips use the custom name while preserving raw identity in the detail view. Randomised MACs aren't tracked (a rotating address isn't a stable device, and tracking it would fill the list with single-use rows, each freshly badged **new**). **DHCP hostname enrichment (live now, #169):** the AP often reports a client's name as `n/a`, so the router's DHCP allocated-address table is merged in by MAC to fill those blanks (e.g. `SMA1930031140`, `energymeter1900241927`, the Amazon/Elgato/Tuya hostnames) — far fewer devices read as "unknown". Wired clients the AP can't see at all are added from the router table too. Which source reported a device (access point, router DHCP, or both) shows as a **Seen by** row in the detail modal, not in the list. The router read is best-effort: if it fails, the AP inventory is unchanged. **Devices with no live link (live now, #550):** a DHCP lease outlives the client that held it, so some of those router-sourced rows are phones that left hours ago — they come back in the read with no band, no SSID and no signal, and the list used to render them as ordinary online devices (16 of 63 "online" rows in one live snapshot). The list now asks for evidence of the link instead: a signal reading, or a wired connection. Anything else is shown as **no link** and rides the same **Offline** toggle — hidden by default, revealed dimmed in a trailing **No link** group (band view) or shaded in place (group view), and excluded from a group header's *online* count so "13/13 online" can't be four absent phones. They are deliberately *not* relabelled offline: the read never established they are gone, and the backend `online` flag (and everything built on it — the history registry, the offline row an *important* device synthesises) keeps meaning "returned by this read". The classification cannot be done server-side — the lease table's `PhyPortName` reads `LAN4` for every client behind the access point, wired or not, and some lease-only rows still answer a ping — so it is the UI that declines to vouch for them. **Ping-probe promotion (live now, #552):** that "some lease-only rows still answer a ping" observation is now acted on — every read's no-link subset (signal-less, non-wired, has an IP) gets a bounded, concurrent host-side ping probe, cached briefly per device so a flapping result doesn't flicker between polls. A device that answers is promoted straight into the live view with its own **Reachable** marker (a distinct badge + a "reachable via ping" signal-slot label) so it's clear the promotion came from a probe, not from AP/router evidence; a device that doesn't answer stays exactly as before, dimmed under **No link**. The probe never touches AP/router evidence for devices that already have it, and a synthesised offline row (genuinely absent from the read) is never probed. **Device groups (live now, #513):** ~55 leases is too long a list to answer *"are all four Elgato lights up?"*, so the device list has a second **Group** mode alongside the band grouping (a segmented **My groups / Band** control, persisted per browser (**My groups** is the default for anyone with nothing persisted yet — issue #519 — a prior explicit **Band** choice is remembered)). In *My groups* each device sits in exactly one user-named group ("Elgato lights", "Cámaras", "Garaje"), the header summarises **online/total** ("Elgato lights · 3/4 online"), and a trailing synthetic **Unclassified** group holds everything unassigned — it always renders last and cannot be renamed or deleted. Groups exist only as the set of assignments, so there is no create-empty step (you create one from the detail modal's **New group…** option) and moving the last device out of a group makes it disappear; the pencil on a real group header renames it (all members follow) or deletes it (members fall back to Unclassified, no device is lost). Grouped rows show **band, SSID and IP** explicitly (the MAC is dropped for consistency with the band view — issue #519 — it's still in the detail modal), and the same offline toggle governs visibility in both grouping modes: with it on, offline members render **shaded, in place**; with it off they're filtered out just like the band view, and a group left with nothing visible simply doesn't render its header. The group's **online/total** count always reflects every member regardless of the toggle. Offline rows keep their **last-known band/SSID** (recorded in the history registry) so they stay readable. These groups are display-only and deliberately **independent of `config/dhcp_plan.json`'s categories**, which exist to drive IP-range assignment for reservations — one taxonomy quietly serving two purposes is how the reservation drift signal gets ruined. Devices behind the garage client bridge report an extender-translated `02:0F:B5:*` MAC, so they are grouped under that derived identity; if such a device is ever moved off the bridge it simply reappears under Unclassified with its real address. Full findings and the follow-up checklist are in [`docs/network-spike.md`](docs/network-spike.md).

- **Endpoints:** `GET /api/network` (one snapshot — `internet` / `access_point` / `router` / `wifi` / `devices`; the `wifi` block is best-effort and carries `available`, current interface/SSID/BSSID/signal/channel/band, visible BSSIDs, custom `display_name`, raw `original_name`, stable `wifi_id`, `hidden`, read-only recommendations, and structured `insights` with channel scores/rationale for later channel-management automation; the `router` block carries `wan_online` / `public_ip` / `gateway` / `dns` / `connection_name` / `uptime_s`; per device the merged `display_name`, `hidden`, OUI `vendor`, `category`, `randomized` flag, the `source` that reported it (`ap` / `router` / `both`), plus the Phase-4 history fields `online` / `first_seen` / `last_seen` / `times_seen` / `important` / `is_new`, the user's display `group` (null = Unclassified), the `last_conn_type` / `last_ssid` an offline row was last seen on, the no-link ping-probe result `ping_reachable` (`true`/`false` for a probed device, `null` when not probed — already has AP/router evidence, or is a synthesised offline row), and synthesised `online:false` rows for known-but-absent devices; an unreachable AP, router, or Wi-Fi scan is reported on its card/block, not an error), `GET /api/network?speedtest=1` (adds an opt-in `speedtest-cli` throughput run, ~13 s — never auto-run), `POST /api/network/access-point/reboot` (reboots the R9000; styled confirm, all clients drop ~1–2 min), `POST /api/network/router/reboot` (reboots the F6600P; styled confirm, ~5-min full outage), `PUT /api/network/devices/{mac}/display_name` (set/clear a device's custom label, persisted to gitignored `config/network_display_names.json` keyed by normalised MAC), `PUT /api/network/devices/{mac}/hidden` (hide/restore an attached device, persisted to gitignored `config/network_hidden.json`), `POST /api/network/devices/{mac}/important` (mark/unmark a device important — the flag lives in the history registry and an important device is never auto-pruned), `PUT /api/network/devices/{mac}/group` (assign a device to a display group; empty clears it back to Unclassified, persisted to gitignored `config/network_groups.json` keyed by normalised MAC), `PUT /api/network/groups/rename` (`{"name": "…", "new_name": "…"}` — moves every member; renaming onto an existing name merges the two), `POST /api/network/groups/delete` (`{"name": "…"}` — drops the assignments only, its devices fall back to Unclassified), `PUT /api/network/wifi/display_name` (set/clear a Wi-Fi label, persisted to gitignored `config/network_wifi_display_names.json` keyed by BSSID or explicit `SSID:<name>` fallback), and `PUT /api/network/wifi/hidden` (hide/restore a Wi-Fi row, persisted to gitignored `config/network_wifi_hidden.json`).
...
The Wi-Fi diagnostics tile above scans from the **dashboard PC** (`netsh wlan`), so it only ever describes coverage where that PC sits. The **Walk test** card answers the question the house actually poses — *where is Wi-Fi bad* — by recording a coverage sample per room, from wherever you are standing with the phone.
...
Endpoints: `GET`/`POST /api/network/survey`, `POST /api/network/survey/delete` (the room label is carried in the body, not the path — it's free-form text that may contain slashes), and `GET /api/network/survey/payload?bytes=N` (incompressible `no-store` bytes for the throughput probe, 2 MiB default, 8 MiB cap). Samples live in `webapp/network_survey.sqlite3` (gitignored, 365-day retention) via `src/network_survey.py`; the live telemetry lookup is `resolve_wireless_client_by_mac` in `src/network_client.py`, which reads the AP **and** the router and merges them — the AP reports a client of the router's radio as `wired` at 100% (an uplink artefact), so an AP-only lookup would fabricate a perfect reading for every sample taken on the router's radio.
...
- **Hide a spare terminal:** the dialog's **Hidden** toggle puts a channel with no clamp fitted away, and a `Show hidden (N)` button brings them all back. It is strictly a user decision — a channel is *never* hidden for reading 0 W, and a hidden one keeps being measured and returned by the API, so fitting a clamp later needs only an unhide.
...
- **Add a newly-paired device — and recover a stale one:** tap **Add** (issue #612), the in-app replacement for a `tinytuya wizard` terminal run. Pair the plug in the Smart Life app, tap the button, confirm, and it appears on the Plugs card. It runs **two** steps because the cloud and the LAN each know half the story: `src/tuya_cloud.py` calls the Tuya Cloud for the account's device list — the only place a device's **local key** and DPS mapping can come from — then runs a TinyTuya UDP broadcast scan (no cloud, no keys) that finds the **current** IP of every powered-on plug and reconciles those addresses into `devices.json` by device id. The scan is **unconditional**, so this is also the **DHCP-churn self-heal**: a plug that merely took a new lease becomes controllable again from the same button. Confirmed before it runs and never on a page load or poll tick — a cloud round trip plus an ~8 s scan is not a background cost; the button shows **Syncing…** while it works and reports exactly what it added and recovered. The sync is **non-destructive**: the cloud is authoritative only for identity and capability (key, mapping, name, category, model, MAC/UUID/SN), while `ip`, `version`, and any captured `dps` block on an existing row are preserved. That merge is `tuya_cloud.py`'s reason to exist — persisting TinyTuya's own `getdevices()` output instead would rebuild "changed" rows from a field whitelist with no `ip`, which on this repo's real `devices.json` dropped the stored address of 4 of the 7 devices that had one.
  > The separate **Refresh** button was retired in #612: the tab already re-polls every ~15 s while open, so its only unique job — the LAN rescan — folded into **Add**. That also keeps the toolbar (**Add** · **Reachable only** · **Show hidden**) on one line down to a 375 px iPhone SE.
- **Endpoints:** `GET /api/tuya` (device cards with switch state, reachability, live energy fields, the `display_name` override, and the `hidden` flag — the per-device LAN reads run in parallel), `POST /api/tuya/pair` (cloud sync + unconditional LAN rescan — the only rediscovery path since `POST /api/tuya/refresh` was retired in #612; 503 with actionable text when credentials are missing or the IoT Core subscription expired), `POST /api/tuya/{id}/switch` (`{"on": true|false}`), `POST /api/tuya/{id}/cover` (`{"action": "open"|"close"|"stop"}`), `PUT /api/tuya/{id}/display_name` (`{"display_name": "…"}`; empty clears the override), `PUT /api/tuya/{id}/hidden` (`{"hidden": true|false}`).
...
- **Missing or stale devices?** Neither case needs a terminal any more — stale **IPs** and **new devices / rotated local keys** both arrive via **Add** (above). The TinyTuya CLI is now only the first-ever bootstrap on a machine with no `devices.json` at all:

  ```powershell
  & .\.venv\Scripts\python.exe -m tinytuya wizard      # first-ever bootstrap only
...
Alexa-style **wake alarms** and countdown **timers**, managed from a collapsible card on the Home tab (after the Home Assistant card, before the Activity log). This is deliberately **separate from the RISCO "Alarm controls"** card — a wake alarm rings/notifies at a set time, it never arms or disarms the security system (issue #304, Step 1/2; the voice-control wiring is Step 2/2, #306).
...
The existing neutral **Home Assistant** VM tile after Energy is now one disclosure, collapsed by default. Its summary row is the whole VM surface (#461): the live `online · up …` / `off` / unavailable status text plus the VM **power switch** (start, or confirm-gated stop, without opening the card; IP·MAC and stale-snapshot detail sit in the status text's hover tooltip). Opening the card shows only Presence-style folded-by-default subsections: **Voice satellites** (every `assist_satellite` Home Assistant currently owns), **Recent interactions**, **What can I do?** (the capabilities guide, formerly a modal), and **What can I say?** (the #437 cheat sheet). Names and room assignments come from HA's entity/device/area registries—there is no duplicate rename store here. Each row shows room, satellite activity, companion media-player volume, and a 44 px microphone control.
...
Home Assistant runs as a **Hyper-V VM** on this PC (TOWER), bridged onto the LAN so it has its own MAC and DHCP-reserved IP. The **Home Assistant disclosure after Energy** shows that VM at a glance while folded — `online · up 3d 4h` (or `off`) — with the Start/Stop switch right on the same summary row (#461), no need to open the card. `GET /api/hyperv` reads the state; `POST /api/hyperv/{start|stop}` controls it. The VM is addressed **by name** (`HA_VM_NAME`), so the card is independent of its IP, and the backend only ever touches that one VM — never the host's other VMs (e.g. WSL2's hidden utility VM). **Stop is a graceful ACPI shutdown** (confirm-gated in the UI); there is no hard power-off.
