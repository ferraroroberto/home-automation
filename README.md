...
- **`app/webapp/`** — **the product**: FastAPI (`server.py` + `middleware.py` + `routers/`) over the same core, serving the static PWA under `static/`, plus the lifecycle `manager.py` and the background tasks (energy sampler, HVAC/security/presence/wake-alarm automation, power monitor, SearXNG watchdog) owned by the app lifespan.
...
## iCloud Find My presence source

Apple Find My is the presence **read** path: a per-account diagnostic locate for every configured Apple Account, and the corroborator that lets a stale webhook state still count as fresh for auto arm/disarm (#653). The **write** path — what actually triggers presence automation — is the iOS Shortcuts webhooks documented in the next section. There is no official Find My API, so this repo reads iCloud's web Find My surface through `pyicloud` (issue #86).
...
**Self-heal, and one alert only when it is stuck (issues #655, #657, #678).** A broken account (`2fa_required`/`error`) no longer needs a manual `tray.bat --restart` to recover — the refresher retries a fresh Apple sign-in on its own, at most once per `PRESENCE_ICLOUD_RETRY_BACKOFF_S` (default 4h) *from the last forced retry, not from the last recovery* — a flapping account stays throttled instead of every recovery resetting the clock (#656) — deliberately much slower than the normal `PRESENCE_ICLOUD_REFRESH_INTERVAL_S` poll so this can never regress into #651's every-poll re-authentication. **One failed poll is not breakage** (#678): Apple answers the Find My fetch with a transient 409 (*"Authentication required for Account."*) often enough that reacting to a single one threw away a working, *trusted* session for nothing. Both reactions are gated on a consecutive-failure streak instead — `PRESENCE_ICLOUD_SELF_HEAL_AFTER_FAILURES` (default 2) before the silent handshake, `PRESENCE_ICLOUD_ALERT_AFTER_FAILURES` (default 4) before the household hears anything - at the default poll the 2nd failure lands ~15 min into a break and the 4th ~45 min in, so nothing is said for the first three quarters of an hour. The handshake itself is silent: the old "Reconnecting … approve the sign-in if prompted" heads-up existed to explain an Apple push that, since #658's `request_2fa_push=False`, this process can no longer cause — it was warning about a prompt that never arrives, and it fired for hiccups that healed themselves two polls later. What is left is **one actionable Telegram message per broken episode** (not per poll) once an account is stuck well past the point self-healing could have worked, naming the account (`ICLOUD_LABEL`/`ICLOUD_LABEL_2`, e.g. "Roberto"/"Ana" — falls back to the Apple ID email when unset, never a bare "account 1"/"account 2"), how long it has been failing, Apple's own reason, and the remedy that actually exists (**Presence → Renew trust**, or the `ICLOUD_PASSWORD` in `.env`). Its paired `✅ … connection restored` fires only if that alert actually went out, so a recovery never arrives without the break it closes. Sent via the same notifier as the alarm/UPS alerts (`config/notify_config.json` or `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`). A healthy account is untouched by any of this, and a transient wobble is now completely silent.
...
| `PRESENCE_ICLOUD_RETRY_BACKOFF_S` | Optional, default `14400` (4h); minimum seconds between self-heal retry attempts for an account stuck in `2fa_required`/`error` (#655). Healthy accounts are unaffected. |
| `PRESENCE_ICLOUD_SELF_HEAL_AFTER_FAILURES` | Optional, default `2`; consecutive failed refreshes before an account earns a forced fresh sign-in, so a single transient Apple 409 never throws away a working session (#678). |
| `PRESENCE_ICLOUD_ALERT_AFTER_FAILURES` | Optional, default `4` (~45 min into a break at the default poll); consecutive failed refreshes before the one actionable Telegram alert per broken episode goes out (#678). Deliberately well past the self-heal threshold — a streak that long is a stuck account, never a hiccup. |
| `ICLOUD_LABEL` / `ICLOUD_LABEL_2` | Optional friendly names (e.g. `Roberto` / `Ana`) used only in the Telegram messages above; unset falls back to that account's Apple ID email. |
...
- **Device row** — category glyph and friendly label (custom name → OUI vendor → reported hostname → MAC), band, SSID, IP, online/offline with "last seen Xh ago" on absent devices, a **new** badge for the first 24 h, and a **Reachable** badge on a device the ping probe promoted. Tapping it opens a detail modal: vendor, IP, band, signal, SSID, MAC, the **Seen by** source (access point / router DHCP / both), a rename field, and persisted **Mark important**, **Hidden** and group controls. An important device is never auto-pruned from the history registry.
- **Visibility** — an **Offline** toggle, always labelled with what is currently visible (**Offline shown** / **Offline hidden**, never the pending action), and a header **Show hidden** pill. Hidden and offline rows are dimmed in place rather than removed while the respective control is on; offline rows keep their last-known band/SSID so they stay readable.
- **Wi-Fi diagnostics** — a collapsible tile reading the dashboard PC's own Windows Wi-Fi interface (`netsh wlan`): current SSID/signal/channel, visible BSSIDs with signal %, band/channel and emitter MAC, and separate 2.4/5 GHz channel-overlap charts in the same translucent line/area style as the solar charts. Wi-Fi rows have their own label and **Hidden** switch; labels and chart tooltips use the custom name while the detail view keeps the raw SSID/BSSID.

Three behaviours are worth knowing because they are not what a naive reading of the data would give. Vendors come from a bundled, trimmed OUI table (offline, slightly stale — an unknown prefix just falls back to hostname/MAC), and randomised per-network MACs are flagged, left un-vendored, and never tracked in history: a rotating address is not a stable device. A device the read returns with an IP but no signal, no SSID and no wired link is shown as **no link** rather than offline — a DHCP lease outlives the client that held it, and the read never established that the device is gone; a bounded host-side ping probe promotes the ones that still answer. And display groups are deliberately independent of `config/dhcp_plan.json`'s categories, which exist to drive IP-range assignment for reservations. The reasoning behind all three, and the rest of the spike findings, is in [`docs/network-spike.md`](docs/network-spike.md).

- **Endpoints:** `GET /api/network` (one snapshot — `internet` / `access_point` / `router` / `wifi` / `devices`; the `wifi` block is best-effort and carries `available`, current interface/SSID/BSSID/signal/channel/band, visible BSSIDs, custom `display_name`, raw `original_name`, stable `wifi_id`, `hidden`, read-only recommendations, and structured `insights` with channel scores/rationale for later channel-management automation; the `router` block carries `wan_online` / `public_ip` / `gateway` / `dns` / `connection_name` / `uptime_s`; per device the merged `display_name`, `hidden`, OUI `vendor`, `category`, `randomized` flag, the `source` that reported it (`ap` / `router` / `both`), plus the history-registry fields `online` / `first_seen` / `last_seen` / `times_seen` / `important` / `is_new`, the user's display `group` (null = Unclassified), the `last_conn_type` / `last_ssid` an offline row was last seen on, the no-link ping-probe result `ping_reachable` (`true`/`false` for a probed device, `null` when not probed — already has AP/router evidence, or is a synthesised offline row), and synthesised `online:false` rows for known-but-absent devices; an unreachable AP, router, or Wi-Fi scan is reported on its card/block, not an error), `GET /api/network?speedtest=1` (adds an opt-in `speedtest-cli` throughput run, ~13 s — never auto-run), `POST /api/network/access-point/reboot` (reboots the R9000; styled confirm, all clients drop ~1–2 min), `POST /api/network/router/reboot` (reboots the F6600P; styled confirm, ~5-min full outage), `PUT /api/network/devices/{mac}/display_name` (set/clear a device's custom label, persisted to gitignored `config/network_display_names.json` keyed by normalised MAC), `PUT /api/network/devices/{mac}/hidden` (hide/restore an attached device, persisted to gitignored `config/network_hidden.json`), `POST /api/network/devices/{mac}/important` (mark/unmark a device important — the flag lives in the history registry and an important device is never auto-pruned), `PUT /api/network/devices/{mac}/group` (assign a device to a display group; empty clears it back to Unclassified, persisted to gitignored `config/network_groups.json` keyed by normalised MAC), `PUT /api/network/groups/rename` (`{"name": "…", "new_name": "…"}` — moves every member; renaming onto an existing name merges the two), `POST /api/network/groups/delete` (`{"name": "…"}` — drops the assignments only, its devices fall back to Unclassified), `PUT /api/network/wifi/display_name` (set/clear a Wi-Fi label, persisted to gitignored `config/network_wifi_display_names.json` keyed by BSSID or explicit `SSID:<name>` fallback), and `PUT /api/network/wifi/hidden` (hide/restore a Wi-Fi row, persisted to gitignored `config/network_wifi_hidden.json`).
...
**Supervised since #716.** The container is watched by the webapp (`app/webapp/searxng_watchdog.py`, a 120-second lifespan loop): a stopped stack is brought back automatically via the same code path as the Start button, and one that is running but not answering `/healthz` is restarted after a ~6-minute grace window — the failure shape no Docker `restart:` policy catches. Attempts back off exponentially (2-minute base, 30-minute cap) and log only on state changes. This exists because the container once sat stopped for eight days while the puck answered current-affairs questions from the model's training cutoff, confidently and with no caveat — a wrong answer rather than a missing one. Details: `docs/voice-control.md` "Backend supervision".
...
- **Unit tests** cover the `src/` domain logic, one module per concern —
  presence (client, engine, automation, refresher), the network stack, cameras,
  Tuya, UPS, voice, telemetry, tariff and energy history, `ha_config_sync`,
  push notifications, PV forecast and the rest — driving the real code against
  `tmp_path` fixtures with every cloud fetcher monkeypatched.

Together that is ~1,170 tests across 93 modules (2026-08-20). Treat the number
as indicative and re-count rather than trusting it — add `--collect-only -q` to
