...
- **`src/`** — non-UI Python.
  ...
  - `alarm_notify_prefs.py` — seven per-event toggles for alarm Telegram alerts (arm/disarm successes default off; `error`/`intrusion`/`ac_lost` default on), persisted to gitignored `config/alarm_notify_prefs.json` via the `display_names` atomic store.
  - `power_notify_prefs.py` — two toggles (`power_lost` / `power_restored`, both default on) for UPS mains-power Telegram alerts, persisted to gitignored `config/power_notify_prefs.json`.
...
- **`app/webapp/`** — the FastAPI + PWA product.
  ...
  - `power_monitor.py` — background task (webapp lifespan) that polls the UPS every `POWER_MONITOR_POLL_INTERVAL_S` in a worker thread and fires `power_notify.record_power_event()` on each mains↔battery transition (the server-side watcher behind the power-loss Telegram alerts).
  - `power_notify.py` — `record_power_event()`: appends UPS power transitions to `logs/power.jsonl` and sends a Telegram alert per the `power_lost`/`power_restored` toggle.
...
- **`custom_components/home_automation_app/`** — Home Assistant custom integration (#235): a thin adapter over the existing `/api/*` endpoints that exposes native `climate`, `switch`, `alarm_control_panel`, `binary_sensor`, and `sensor` entities. See [`docs/home-assistant-integration/`](docs/home-assistant-integration/README.md).
