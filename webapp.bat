...
if not exist "%CERT%" (
    ...
    "%VENV_PY%" -m uvicorn app.webapp.server:app --host 0.0.0.0 --port 8447
) else (
    echo [INFO] HTTPS via %CERT%
    "%VENV_PY%" -m uvicorn app.webapp.server:app --host 0.0.0.0 --port 8447 --ssl-keyfile "%KEY%" --ssl-certfile "%CERT%"
)
