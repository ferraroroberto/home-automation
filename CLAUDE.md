...
- `src/risco_client.py` — async, UI-free RISCO Cloud alarm core. `fetch_security_state()` returns the flattened `SecurityState` (system flags incl. **system-wide `battery_low`** + per-zone `trouble`, partitions, zones); arm/disarm/bypass go through the native WebUI command path. The cloud API exposes **no per-detector battery** — only the aggregate `battery_low` flag (drives the UI low-battery alert) and a generic per-zone `trouble` boolean (issue #84).
...
- `scripts/` — `gen_tailscale_cert.py` (HTTPS via `tailscale cert` — a real Let's Encrypt leaf for the tailnet `.ts.net` name; `--check` auto-renews when within 30 days of expiry), `gen_token.py` / `set_password.py` (auth), `gen_icons.py` (PWA icons; Pillow dev-only).
