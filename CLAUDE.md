...
**Interacting with the Home Assistant VM — SSH first, browser only as fallback (#243).** Whenever you need to read or change Home Assistant config (`/config/configuration.yaml`, `custom_sentences/`, automations, `secrets.yaml`), run `ha core check`/reloads, restart HA, or text-probe the conversation engine, do it **over SSH** — via `scripts/ha_config_sync.py` for the repo-owned voice-PE config (`preflight` / `deploy [--dry-run] [--restart]` / `rollback` / `probe`), or a direct `ssh -p <HA_SSH_PORT> <HA_SSH_USER>@<HA_SSH_HOST>` (the **Terminal & SSH add-on** shell, which mounts `/config` + the `ha` CLI) for one-off commands and arbitrary file edits. Connection settings live in `.env` (`HA_SSH_*`, `HA_URL`, `HA_TOKEN`). Do **not** drive the HA File editor / Developer Tools through Chrome automation when SSH can do it — the browser path is the documented fallback for when the add-on is down or a key isn't provisioned. The one-time SSH bootstrap is in `docs/voice-pe-config/README.md`. (HAOS host SSH on `:22222` is break-glass only — not the path here.)
...
**Security model:** the webapp binds `0.0.0.0:8447` and is reached over **Tailscale** behind a **real Let's Encrypt HTTPS** endpoint (`tailscale cert`, issued for the tailnet `.ts.net` name — no per-device trust) + an optional **bearer token** (loopback bypasses; remote needs `Authorization: Bearer` or `?token=`). The tray's **Open** action targets the trusted `.ts.net` URL so the PC needs no certificate either. Mirrors the `photo-ocr` / `app-launcher` stack. No cloudflared tunnel, no WebAuthn passkey (there's no terminal here).
