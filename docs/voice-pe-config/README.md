...
**How the follow-up works (#470).** After the locate answer, if the person is **away** the puck asks *"Do you want to know how long it'll take to get home?"* — say **yes** and it speaks a traffic-aware ETA (Google **Routes API**, `routingPreference: TRAFFIC_AWARE`) from their live coordinates to the home in `config/location.json`. Already home → it just says so, no prompt. Every failure mode (no location, home not set, no API key, no route) degrades to a spoken fallback, never an error — the same graceful contract as locate.
