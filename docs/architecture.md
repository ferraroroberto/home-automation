...
- `_env.py` — shared `.env` knob parsers (`_env_bool` and friends) for the webapp's background-task modules (`presence_automation` / `presence_refresher` / `security_automation` / `automation`), so the "blank → default, invalid → warn-and-default" behaviour can't drift between the copy-pasted versions each loop used to carry.
