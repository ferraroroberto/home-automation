...
- `power_notify_prefs.py` — three toggles (`power_lost` / `power_restored` / `auto_shutdown_low_battery`, all default on) for UPS mains-power Telegram alerts and the low-battery safety auto-shutdown, persisted to gitignored `config/power_notify_prefs.json`.
