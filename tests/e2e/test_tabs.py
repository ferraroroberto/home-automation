...
an Energy-tab render (hero numbers + charts) against stubbed energy fixtures —
on both the Chromium-desktop and WebKit/iPhone projections.
"""
...
def test_tab_navigation_switches_panes(
    ...
    # Energy tab → the live hero row shows.
    page.locator("#tabEnergy").click()
    expect(page.locator("#paneEnergy")).to_be_visible()
    expect(page.locator("#paneAc")).to_be_hidden()
    expect(page.locator(".energy-heroes")).to_be_visible()
...
def test_energy_tab_renders_heroes_and_charts(
    page: Page, base_url: str, sample_units: List[Dict],
    mock_api: Callable, mock_energy: Callable,
) -> None:
    mock_api(sample_units)
    mock_energy()  # default fixture: PV 2500 W, house 1300 W, net +1200 W
    _boot(page, base_url)
    page.locator("#tabEnergy").click()

    expect(page.locator("#heroProd")).to_have_text("2500 W")
    expect(page.locator("#heroCons")).to_contain_text("1300")
    expect(page.locator("#heroNet")).to_contain_text("1200")
