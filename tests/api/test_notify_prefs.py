...
and the "configured" probe is faked — no real config, no Telegram. Both routes
are generated by ``routers/_helpers.make_bool_prefs_router`` (#664), so the
probe is patched there rather than on either router module.
"""
...
def test_notify_prefs_round_trip_and_configured_flag(
    ...
    monkeypatch.setattr(
        "app.webapp.routers._helpers.is_notify_configured", lambda: True
    )
...
def test_notify_prefs_unconfigured_reports_false(
    ...
    monkeypatch.setattr(
        "app.webapp.routers._helpers.is_notify_configured", lambda: False
    )
...
def test_power_notify_prefs_round_trip(client: TestClient, monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("app.webapp.routers._helpers.is_notify_configured", lambda: True)
