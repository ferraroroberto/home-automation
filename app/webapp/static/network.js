...
// ------------------------------------------------- DHCP reservation manager (#170/#176)
// Lazy-loads GET /api/network/dhcp-plan on first open, then on demand via Refresh.
// The F6600P caps the static-binding table at 10 rows, so this is a small *staged*
// reservation manager, not a one-shot apply: mark router rows to remove, tick
// suggested/manual rows to add, then one "Apply changes" runs the removes then the
// adds in a single router session (POST /api/network/dhcp-reservations/apply). Every
// write is confirm-gated and never runs on a poll.
