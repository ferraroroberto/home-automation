...
# ------------------------------------------------ iCloud browser-trust renewal
# (issue #659) Two-step, attended: begin makes Apple push a 2FA code to the
# account's trusted devices; complete verifies it on the SAME live pyicloud
# session (see src.presence_client) and swaps the trusted session into the
# tray's cache — no restart, no CLI. Same auth as every /api/* route.
